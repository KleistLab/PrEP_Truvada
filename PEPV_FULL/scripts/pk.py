...
class PharmacokineticsTDF(AbstractPharmacokinetics):
    ...
    def _generate_sample_parameters(self):
        ...
        if self._sample_file is not None:
            print('TFV',type(self._sample_file))
            # Check if the input is a file path (string) or a DataFrame
            if isinstance(self._sample_file, str):
                # Read CSV file if it's a file path
                df = pd.read_csv(self._sample_file, index_col='ID', dtype=float)
                print('PK parameters created from file.')
            elif isinstance(self._sample_file, pd.DataFrame):
                #and (isinstance(self._sample_file, pd.DataFrame) and not self._sample_file.empty)
                # Use the DataFrame directly if it's already provided
                df = self._sample_file.drop(columns=['ID'], errors='ignore')
                print('PK parameters created from provided DataFrame.')
            else:
                raise ValueError("Invalid input: _sample_file should be a file path or a DataFrame.")
            
            # Convert the DataFrame to a tensor
            param = torch.tensor(df.iloc[:, :].values, dtype=torch.double)
        
        else:  # Default: median of the PK parameters of 1000 individuals
            param = torch.tensor([[9.24, 0.631, 0.396, 0.016, 1.3e-2, 0.1313, 360.94]], dtype=torch.double)
...
class PharmacokineticsFTC(AbstractPharmacokinetics):
    ...
    def _generate_sample_parameters(self):
        ...
        param: double ndarray (n_samples, 9), [Ka, CL, Vmax, Km, CLp, CLd, V2, V3, V4]
        """
        print('FTC',type(self._sample_file))
        ...
        if self._sample_file is not None:
            ...
            if isinstance(self._sample_file, str):
                # Read CSV file if it's a file path
                df = pd.read_csv(self._sample_file, index_col='ID', dtype=float)
                print('PK parameters created for FTC from file.')
            elif isinstance(self._sample_file, pd.DataFrame):
                # Use the DataFrame directly if it's already provided
                df = self._sample_file.drop(columns=['ID'], errors='ignore')
                print('PK parameters created for FTC from provided DataFrame.')
            else:
                raise ValueError("Invalid input: _sample_file should be a file path or a DataFrame.")
            ...
            print('median pop PK parameters are used')
            param = torch.tensor([[0.596, 21.87, 6.93, 11.4, 0.74, 7.482, 60.40, 0.989, 118.515]], dtype=torch.double)
...
class PharmacokineticsEFV(AbstractPharmacokinetics):
    ...
    def _generate_sample_parameters(self):
        ...
        """
        if self._sample_file:
            df = pd.read_csv(self._sample_file, dtype=float)
            param = torch.tensor(df.iloc[:, [2, 1, 0]].values, dtype=torch.double)
        else:       # median of the PK parameters of 1000 individuals
    ...
    def _compute_concentration(self):
        ...
        d_array = None  # initiate variable for later use.
        t0_idx = 0
        while t0_idx < regimen_matrix.shape[1] and not regimen_matrix[0, t0_idx]:        
            t0_idx += 1
        if t0_idx >= regimen_matrix.shape[1]:       # if no drug is taken
            t0_idx = 0
...
class PharmacokineticsISL(AbstractPharmacokinetics):
    """
    PK model for FTC (of female), from paper Garrett et al. PMID: 30150483
    unit [uM]
    """
    ...
    def __init__(self, regimen, sample_file=None, ratio=1, ode_solver=euler, time_step=0.02):
        ...
        self._coefficient_matrix = self._generate_pk_coefficient_matrix()
        self._concentration_profile_whole = self._compute_concentration()
        self._concentration_profile_target = self._concentration_profile_whole[..., [6]]
        self._slice_concentration_array()

    def _generate_sample_parameters(self):
        """
        :return:
        param: double ndarray (n_samples, 13) [K, KR, Kim, K20, Ka, Qic, Vc, Kc1, K1c, K10, Kc2, K2c, K30]
        """
        if self._sample_file:
            df = pd.read_csv(self._sample_file, index_col='ID', dtype=float)
            param = torch.tensor(df.iloc[:, 0:].values, dtype=torch.double)
        else:  # the pk parameters depend on administration and dosing
            param = torch.tensor([[0] * 6 + [3.5, 2.6, 0.003, 7.7, 4.6, 0.03, 0.009]], dtype=torch.double)
            if self.regimen.get_administration() == 'oral':
                param[0, 4] = 0.4
                dose_list = [0.5, 1, 2, 5, 10, 15, 30, 100, 200, 400]
                qic_list = [43.24453, 32.06647, 19.02982, 14.4065, 20.10468, 15.70732, 19.96877, 9.95159, 11.91026,
                            10.33444]
                qic_dict = {d: q for d, q in zip(dose_list, qic_list)}
                if self.regimen.get_dose() in qic_dict:
                    param[0, 5] = qic_dict[self.regimen.get_dose()]
                else:
                    sys.stderr.write('PK parameter unknown: ISL oral dose invalid')
            else:  # implant
                param[0, 0] = 0.005241 / 24
                param[0, 2] = 0.02
                param[0, 3] = 0.2
                if self.regimen.get_dose() == 54:  # implant 54mg
                    param[0, 1] = 0.007367 / 24
                    param[0, 5] = 21.63493
                elif self.regimen.get_dose() == 62:  # implant 62mg
                    param[0, 1] = 0.011049 / 24
                    param[0, 5] = 38.4584
                else:
                    sys.stderr.write('PK parameter unknown: ISL implant dose invalid')
        return param

    def _generate_pk_coefficient_matrix(self):
        """
        :return:
        term: double ndarray (n_samples, [n_regimen, ], 7, 7) [R, IM, D, C, P1, P2, IC]
        """
        term = torch.zeros([self._samples.shape[0], 7, 7], dtype=torch.double)
        term[:, 0, 0] = -self._samples[:, 1]
        term[:, 1, 0] = self._samples[:, 1]
        term[:, 1, 1] = -self._samples[:, 2] - self._samples[:, 3]
        term[:, 2, 2] = -self._samples[:, 4]
        term[:, 3, 1] = self._samples[:, 2]
        term[:, 3, 2] = self._samples[:, 4] / self._samples[:, 6]
        term[:, 3, 3] = -self._samples[:, 7] - self._samples[:, 9] - self._samples[:, 10]
        term[:, 3, 4] = self._samples[:, 8]
        term[:, 3, 5] = self._samples[:, 11]
        term[:, 4, 3] = self._samples[:, 7]
        term[:, 4, 4] = -self._samples[:, 8]
        term[:, 5, 3] = self._samples[:, 10]
        term[:, 5, 5] = -self._samples[:, 11]
        term[:, 6, 3] = self._samples[:, 5]
        term[:, 6, 6] = -self._samples[:, 12]
    ...
    def _pk_model(self, t, c):
        ...
        :parameter:
        t: double
        c: double array (n_samples, [n_regimen, ], 7, 1) [R, IM, D, C, P1, P2, IC]
        :return:
        model: double ndarray (n_samples, [n_regimen, ], 7, 1)
        """
        coefficient_matrix = self._coefficient_matrix.clone()
        coefficient_matrix[..., 0, 0] = coefficient_matrix[..., 0, 0] * np.exp(-self._samples[..., [0]] * t)
        coefficient_matrix[..., 1, 0] = coefficient_matrix[..., 1, 0] * np.exp(-self._samples[..., [0]] * t)
        return torch.matmul(coefficient_matrix, c)
    ...
    def _compute_concentration(self):
        ...
        regimen_matrix = self.regimen.get_regimen_matrix()
        molecular_weight = self.regimen.get_molecular_weight()
        d0 = torch.zeros([self._samples.shape[0]] + list(regimen_matrix.shape)[:-1] + [7, 1], dtype=torch.double)
        d_array = None  # initiate variable for later use.
        if self.regimen.get_administration() == 'oral':
            for i in range(regimen_matrix.shape[-1]):
                d0[..., 2, 0] = d0[..., 2, 0] + regimen_matrix[..., i] * 1e3 / molecular_weight
                d_array = self._ode_solver(self._pk_model, i * period, (i + 1) * period, d0, self._time_step)
                d0 = d_array[-1].clone()
                tmp_concentration.append(d_array[:-1:self._ratio])  # ignore the last point to avoid duplicate
            tmp_concentration.append(d_array[[-1]])  # add the last point of the last iteration
            concentration = torch.cat(tmp_concentration).squeeze(dim=-1)
        else:       # implant with different dose
            init_conc = 0
            if self.regimen.get_dose() == 54:
                init_conc = 0.041
            elif self.regimen.get_dose() == 62:
                init_conc = 0.058
            for i in range(regimen_matrix.shape[-1]):
                d0[..., 0, 0] = d0[..., 0, 0] + regimen_matrix[..., i] * 1e3 / molecular_weight * (1 - init_conc)
                d0[..., 1, 0] = d0[..., 1, 0] + regimen_matrix[..., i] * 1e3 / molecular_weight * init_conc
                d_array = self._ode_solver(self._pk_model, i * period, (i + 1) * period, d0, self._time_step)
                d0 = d_array[-1].clone()
                tmp_concentration.append(d_array[:-1:self._ratio])  # ignore the last point to avoid duplicate
            tmp_concentration.append(d_array[[-1]])  # add the last point of the last iteration
