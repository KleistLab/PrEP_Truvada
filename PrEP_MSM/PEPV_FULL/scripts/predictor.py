...
class EfficacyPredictor(object):
    ...
    def compute_concentration(self):
        ...
        for regimen, file in zip(self.regimens, self.files):
            ratio = 1       # ratio of default timestep / set-up timestep for PK computation
    ...
    def compute_drug_effect(self, file=None, reservoir=False, macrophage=False):
        """
        compute the drug effect (eta) profile: self.PharmacoDynamicsInterface will be initialized (pd object generated)
        :parameter:
        file: str
    ...
    def compute_extinction_probability(self, pd_file=None, reservoir=False):
        ...
        ATTENTION: here the reservoir indicates if latent and long-lived cells are considered as infection during computation of extinction, so its meaning is different from self._reservoir in PdInterface, and the propensities will also NOT be changed if reservoir=True. Only when computing extinction probability in class PgSystemExtinction the reservoir will be considered. 
        """
        self.compute_drug_effect(pd_file)
    ...
    def compute_extinction_probability_fullmodel(self, pd_file=None):
        """
        Compute the extinction probability profile based on the full VD model (macrophage cycle and latent reactivation and proliferation considered)
        """
        self.compute_drug_effect(pd_file, reservoir=True, macrophage=True)
        if self._pgs_class is not PgSystemExtinctionFull:
            self._set_pgs_class(PgSystemExtinctionFull)
    ...
    def compute_infection_probability(self, pd_file=None, macrophage=False):
        """
        Compute the infection probability profile and assign the result to self._pi
        :parameter:
        macrophage: boolean, if the cycle of macrophage (reactivation) is considered in the viral dynamics (currently only available for infection probability). If True: the propensities will be changed to include the reactivation and further infection of macrophage.
        """
    ...
    def compute_cumulative_infection_probability(self, macrophage=False):
        ...
        Compute the cumulative probability and assign the result to self._cdf_pe
        :parameters:
        macrophage: same as compute_infection_probability()
        """
